...
samtools view -b | \
samtools sort -@ 10 modBAM.mapped.bam
...
bedMethyl_df = pd.read_csv(args.input1, sep='\t', headers=None)
...
bedMethyl_df = bedMethyl_df[[c for c in bedMethyl_df.columns if not c.startwith("drop")]]
...
for modification in modifications:
    ...
    modification_table.to_csv(os.join.path("modification_tables", modification_names[modification] + ".tsv"), sep='\t', index=False)
```
...
vcf_df = pd.read_csv(args.input1, sep='\t' usecols=['Contig', 'Position'])
...
srun conda run -p "path/to/modkit_env" modkit find-motifs \
-i path/to/pileup.bed \
-r path/to/.fna \
-o output/path/motifs.tsv \
-t 32 \
```
...
# %% Argparse preamble
parser = argparse.ArgumentParser(description="Generate a consensus plot given a modification table TSV file from the bedMethyl file")
parser.add.argument("input1", help="Provide the path to a modification table TSV file (annotation file okay), e.g., 6mA.tsv")
parser.add.argument("input2", help="Provide the path to the FASTA file (FA or FNA file okay)")
args = parser.parse_args()

# %% Import the TSV input as a DataFrame and only keep the 'Position' column
consensus_df = pd.read_csv(args.input1, sep"\t", usecols=['Position'])

# %% Extract the sequence around the each of the modification positions

# Parameters
filename = args.input2 # import the FASTA file path
...
# Filter out all the positions that do not have a sequence length of 24 characters (23 is listed below bc IDK)
consensus_df = consensus_df[(consensus_df["Sequence"].apply(len) == 23)].reset_index(drop=True)

# %% Create the consensus logo
...
# Plot title
title = "Consensus Logo"

# Plot logo
plt.figure(figsize=(10, 4))
logo = logomaker.Logo(counts_df)
plt.title(title)
plt.savefig("consensus_logo.pdf")
```
